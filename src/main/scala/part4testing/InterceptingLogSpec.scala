...
/**
 * This lecture shows how to test more complex actors that have a flow of messaging through differents
 * collaborators (other actors). In these cases, we can use EventFiler.
 *
 * EventFilter.info(pattern, occurences,...) intercepts n occurrences of log messages of level INFO which matchs
 * the given pattern.
 */
